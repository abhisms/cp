...
const AppHeaderDropdownPO = () => {
  ...
  useEffect(() => {
    ...
    const fetchPlacementOfficerDetails = async () => {
      try {
        const response = await axios.get(`https://cp-backend-jx53.onrender.com/api/placement/GetById/${loggedInUserId.replace(/"/g, '')}`);
  ...
  const handleSaveChanges = () => {
    const updatePlacementOfficer = async () => {
      try {
        ...
        const response = await axios.put(`https://cp-backend-jx53.onrender.com/api/placement/update/${placement_officer._id}`, newFormData);
  ...
  return (
    <CDropdown variant="nav-item">
      <CDropdownToggle placement="bottom-end" className="py-0 pe-0" caret={false}>
        <CAvatar
          src={`https://cp-backend-jx53.onrender.com/api/upload/${formData.p_photo}`}
          alt="profile"
      ...
      <CDropdownMenu className="pt-0" placement="bottom-end">
        ...
        <CModal visible={photoModalVisible} onClose={() => setPhotoModalVisible(false)} size="lg">
          ...
          <CModalBody>
            <CImage fluid src={`https://cp-backend-jx53.onrender.com/api/upload/${formData.p_photo}`} alt="Profile" />
          </CModalBody>
          <CModalFooter>

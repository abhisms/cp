...
const AppHeaderDropdownHOD = () => {
  ...
  useEffect(() => {
    ...
    const fetchHodDetails = async () => {
      try {
        const response = await axios.get(`https://cp-backend-jx53.onrender.com/api/hod/GetById/${loggedInUserId.replace(/"/g, '')}`);
  ...
  const handleSaveChanges = () => {
    const updateHod = async () => {
      try {
        ...
        const response = await axios.put(`https://cp-backend-jx53.onrender.com/api/hod/update/${hod._id}`, newFormData);
  ...
  return (
    <CDropdown variant="nav-item">
      <CDropdownToggle placement="bottom-end" className="py-0 pe-0" caret={false}>
        <CAvatar
          src={`https://cp-backend-jx53.onrender.com/api/upload/${formData.h_photo}`}
          alt="profile"
      ...
      <CDropdownMenu className="pt-0" placement="bottom-end">
        ...
        <CModal visible={photoModalVisible} onClose={() => setPhotoModalVisible(false)} size="lg">
          ...
          <CModalBody>
            <CImage fluid src={`https://cp-backend-jx53.onrender.com/api/upload/${formData.h_photo}`} alt="Profile" />
          </CModalBody>
          <CModalFooter>

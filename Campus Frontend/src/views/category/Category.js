...
const Tables = () => {
  ...
  const Edit = ({ item }) => {
    ...
    const handleEdit = () => {
      axios.put(`https://cp-backend-ewpv.onrender.com/api/category/update/${newCategory._id}`, newCategory)
        .then((res) => {
          if (res.data.success) {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this job category?")) {
      axios.delete(`https://cp-backend-ewpv.onrender.com/api/category/delete/${id}`)
        .then((res) => {
          if (res.data.success) {

...
const Tables = () => {
  ...
  useEffect(() => {
    axios.get("https://cp-backend-ewpv.onrender.com/api/placement/get")
      .then((res) => {
        console.log(res)
  ...
  const Edit = ({ item }) => {
    ...
    const handleEdit = () => {
      ...
      axios.put(`https://cp-backend-ewpv.onrender.com/api/placement/update/${newPlacement_officer._id}`, formData)
        .then((res) => {
          if (res.data.success) {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this?")) {
      axios.delete(`https://cp-backend-ewpv.onrender.com/api/placement/delete/${id}`)
        .then((res) => {
          if (res.data.success) {
              ...
              <CTableBody>
                {placement_officer.map((item, index) => (
                  <CTableRow key={item._id}>
                    ...
                    <CTableDataCell><img style={{ height: "100px", width: "100px" }} alt='' src={`https://cp-backend-ewpv.onrender.com/api/upload/${item.p_photo}`} /></CTableDataCell>
                    <CTableDataCell>
                      <Edit item={item} />

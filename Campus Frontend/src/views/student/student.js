...
const Tables = (role) => {
  ...
  useEffect(() => {
    axios.get("https://cp-backend-jx53.onrender.com/api/student/get")
      .then((res) => {
  ...
  const Edit = ({ item }) => {
    ...
    const handleEdit = () => {
      ...
      axios.put(`https://cp-backend-jx53.onrender.com/api/student/update/${newStudent._id}`, formData)
        .then((res) => {
          if (res.data.success) {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this student?")) {
      axios.delete(`https://cp-backend-jx53.onrender.com/api/student/delete/${id}`)
        .then((res) => {
          if (res.data.success) {
              ...
              <CTableBody>
                {student.map((item, index) => (
                  <CTableRow key={item._id}>
                    ...
                    <CTableDataCell><img style={{ height: "50px", width: "50px", objectFit: "cover" }} src={`https://cp-backend-jx53.onrender.com/api/upload/photo/${item.s_photo}`} alt="Student" /></CTableDataCell>
                    <CTableDataCell><a href={`https://cp-backend-jx53.onrender.com/api/upload/resume/${item.s_resume}`} target="_blank" rel="noopener noreferrer">Resume</a></CTableDataCell>

...
const Tables = () => {
  ...
  const Edit = ({ item }) => {
    ...
    const handleEdit = () => {
      axios.put(`https://cp-backend-ewpv.onrender.com/api/branch/update/${newBranch._id}`, newBranch)
        .then((res) => {
          if (res.data.success) {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this branch?")) {
      axios.delete(`https://cp-backend-ewpv.onrender.com/api/branch/delete/${id}`)
        .then((res) => {
          if (res.data.success) {

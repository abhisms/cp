...
const EditJobModal = ({ item, setJobs }) => {
    ...
    useEffect(() => {
        axios.get("https://cp-backend-ewpv.onrender.com/api/category/get")
            .then((res) => {
    ...
    const handleEdit = () => {
        ...
        axios.put(`https://cp-backend-ewpv.onrender.com/api/job/update/${newJobs._id}`, formData)
            .then((res) => {
                if (res.data.success) {
...
const DeleteJobModal = ({ item, setJobs }) => {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this?")) {
            axios.delete(`https://cp-backend-ewpv.onrender.com/api/job/delete/${id}`)
                .then((res) => {
                    if (res.data.success) {
...
const JobsDescription = ({ role }) => {
    ...
    useEffect(() => {
        if (role === 'student') {
            ...
            axios.get(`https://cp-backend-ewpv.onrender.com/api/application/check/${student_id}/${id}`)
                .then((res) => {
        ...
        <CRow>
            <CCol xs={12}>
                <CCard className="mb-4">
                    ...
                    <CCardBody>
                        <br />
                        <CCardImage orientation="top" style={{ height: "200px", width: "200px" }} src={`https://cp-backend-ewpv.onrender.com/api/upload/${jobs.cover_photo}`} />
                        <br />
                        <br />
                        <CCardText>

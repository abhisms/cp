...
function Row(props) {
  ...
  const handleChange = (appId, status) => {
    axios.put(`https://cp-backend-ewpv.onrender.com/api/application/update/${appId}`, { status })
    .then((res)=>{
      console.log(res)
    })
    .catch((err)=>{
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ height: "200px", '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell>
          <a
            href={`https://cp-backend-ewpv.onrender.com/api/upload/${row.resume}`}
            target="_blank"
...
const Applications = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const res = await axios.get('https://cp-backend-ewpv.onrender.com/api/application/all');
        ...
        const detailedApplications = await Promise.all(
          applicationsData.map(async (application) => {
            ...
            const jobRes = await axios.get(`https://cp-backend-ewpv.onrender.com/api/application/job/get/${jobId}`);
            const studentRes = await axios.get(`https://cp-backend-ewpv.onrender.com/api/application/student/get/${studentId}`);
            const branchRes = await axios.get(`https://cp-backend-ewpv.onrender.com/api/application/branch/get/${courseId}`);

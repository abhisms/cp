...
const Tables = ({ role }) => {
  ...
  useEffect(() => {
    axios.get("https://cp-backend-e04k.onrender.com/api/job/get")
      .then((res) => {
        console.log(res);
            ...
            <div className="card-grid">
              {jobs.map((item, index) => (
                <CCard key={item._id} style={{ width: '18rem' }}>
                  <CCardImage orientation="top" style={{ height: "200px" }} src={`https://cp-backend-e04k.onrender.com/api/upload/${item.cover_photo}`} />
                  <CCardBody>
